...
/**
 * This interface extends the bento_domain interface, which correspondes to the bento_domain
 * object defined in core.  It represents a Bento domain, which consists of Bento code loaded
 * under a particular set of restrictions.
 */
public class BentoDomain implements bento_domain {
    ...
    private BentoDomain mainSite;
    private String domainName;
    private String domainType;
    private bento_processor bentoProcessor;
    private Map<String, BentoDomain> childDomains = null;
    ...
    /** Constructs a new root BentoDomain. 
     * 
     *  A BentoDomain is capable of loading and compiling Bento source code
     ...
     */
    public BentoDomain(String name, bento_processor processor) {
        if ("core".equals(name)) {
            throw new IllegalArgumentException("The name \"core\" is reserved.");
        }
        mainSite = this;
        bentoProcessor = processor;
        domainName = name;
        domainType = processor.domain_type();
    }

    /** Constructs a child BentoDomain. 
     */
    private BentoDomain(BentoDomain mainSite, String name, String domainType) {
        if ("core".equals(name)) {
            throw new IllegalArgumentException("The name \"core\" is reserved.");
        ...
        if (mainSite.childDomains == null) {
            mainSite.childDomains = new HashMap<String, BentoDomain>();
        }
        mainSite.childDomains.put(name, this);

        this.mainSite = mainSite;
        this.childDomains = mainSite.childDomains;
        this.bentoProcessor = mainSite.bentoProcessor;
        this.domainName = name;
        this.domainType = domainType;
        this.core = new Core();
    }

    public BentoDomain getChildDomain(String name, String type) {
        BentoDomain child = null;
    ...
    public BentoDomain createChildDomain(String name, String type) {
        return new BentoDomain(this, name, type);
    }
    ...
    private synchronized boolean reload(SiteLoader loader, Core core) {
        ...
        if (!domainName.equals(Name.DEFAULT)) {
            ...
            if (site == null) {
                log("No data for site " + domainName + " in bentopath; creating empty site");
                site = new Site(domainName);
                core.addSite(site);
            }
        } else if (defaultSite == null) {
            log("No data for site in bentopath.");
        }
    ...
    public Object getPropertyCollectionObject(String name) {
        ...
        try {
            ...
            if (collectionObj instanceof BentoArray) {
                collectionObj = ((BentoArray) collectionObj).instantiateArray(context);
            } 
    ...
    void globalInit() {
        if (!globallyInitialized) {
            ...
            addExternalObject(this, "this_domain", "bento_domain");
            addExternalObject(bentoProcessor, "this_processor", "bento_processor");
            addExternalObject(bentoProcessor, "this_server", "bento_server");
    ...
    public String toString() {
        String str = "Bento Domain " + (domainName != null ? domainName : "(no name)") + '\n';
        if (site != null) {
            str += site.toString();
        } else {
            str += "(empty)";
     ...
     *  domain name).
     **/
    public Map<String, BentoDomain> domains() {
        return childDomains;
    }
    ...
    /** Creates a bento_context object (represented by the BentoContext interface), which can
     *  be used to construct objects.  The bento_context will be able to construct objects in
     *  any of the sites in this domain.
     */
    public bento_context context() {
        return new BentoContext(this);
    }
    ...
    public bento_domain child_domain(String name, String type, String src, boolean isUrl) {
        BentoDomain child = createChildDomain(name, type);
        if (!child.isLoaded()) {
