...
public class ExternalDefinition extends ComplexDefinition {
    ...
    //    this(null, (BentoNode) owner, owner, superType, access, dur, object, args);
    //}
...
//   }
    ...
    public List<Dim> getDims() {
        if (dims == null) {
            ...
            } else if (c.isArray() || List.class.isAssignableFrom(c) || BentoArray.class.isAssignableFrom(c)) {
...
//               vlog("No field " + name + " in class " + clazz.getName());
    ...
    public int getObjectSize() {
        ...
        } else if (obj instanceof BentoArray) {
            return ((BentoArray) obj).getSize();
    ...
    static boolean  isCollectionClass(Class<?> clazz) {
    	 return (clazz.isArray() || 
    	         BentoArray.class.isAssignableFrom(clazz) ||
    ...
    /** Returns true if the specified class represents a numeric type.  In Bento,
    ...
    public ExternalConstruction initExternalObject(Context context, ArgumentList args) throws Redirection {
        ...
        for (int i = 0; i < numArgs; i++) {
            ...
            // dereference value generators
                ...
                if ((argVal == null || argVal.equals(NullValue.NULL_VALUE)) && arg instanceof BentoNode) {
                    Definition argOwner = ((BentoNode) arg).getOwner();
                    int numUnpushes = 0;
                    try {
...
//                }
            ...
            } else if (arg instanceof BentoArray) {
                argObjects[i] = ((BentoArray) arg).instantiateArray(context);
