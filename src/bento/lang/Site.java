...
public class Site extends ComplexDefinition {
    ...
    public Definition[] getDefinitions() {
        ...
        synchronized (defTable) {
            Collection defColl = defTable.values();
     ...
     */
    public void addDefinitions(String type, List list) {
        DefinitionHash defTable = (DefinitionHash) getDefinitionTable();
        Iterator it = defTable.values().iterator();
        synchronized (defTable) {
            while (it.hasNext()) {
     ...
     */
    public void addSubdefinitions(String type, List list) {
        DefinitionHash defTable = (DefinitionHash) getDefinitionTable();
        Iterator it = defTable.values().iterator();
        synchronized (defTable) {
            while (it.hasNext()) {
    ...
    /** Implementation of bento_site interface **/
...
//                       put(name, entry);
            ...
            if (entry == null) {
                ...
                return (Definition) ((List) entry).get(0);
