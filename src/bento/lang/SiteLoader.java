/* Bento
 *
 * $Id: SiteLoader.java,v 1.49 2015/07/04 17:41:13 sthippo Exp $
 *
...
import bentocore.CoreSource;
import bento.runtime.BentoVisitor;
import bento.runtime.SiteBuilder;
...
/**
 * A SiteLoader loads a bento path.  Parsing occurs in a background thread.  This
...
public class SiteLoader {
    ...
    private List<BentoSourceLoader> loaders = new ArrayList<BentoSourceLoader>();
    protected Core core;
    ...
    /** Load the Bento source code for this site.  The loading occurs in three
     *  steps:
     *  <ol>
     *  <li>If config.bento is present as a resource in the classpath, it is loaded; else all .bento files in the externally specified bentopath are loaded.</li>
     *  <li>If there is a definition for "bentopath" in a file loaded in the first step,
     *  all .bento files in the path it specifies are loaded.</li>
     *  <li>If the Bento core was not loaded after the first two steps, and the loadCore
     *  property for this site is true, then the core bento files are loaded from the
     *  corepath, if a definition for "corepath" has been loaded, or else as a resource
     *  from the bentocore directory in the classpath.</li>
     */
    public void load() {
        String configPath = "/config.bento";
        Class<?> c = getClass();
        URL url = null;
        if (externalUrl != null) {
        ...
        // core.bento and core_platform_java.bento using the loader from this class,
        // which will most often be loading from bento.jar
        if (loadCore) {
        ...
        synchronized (loadedFiles) {
            ...
            for (int i = 0; i < size; i++) {
                BentoSourceLoader loader = loaders.get(i);
    ...
    private void loadFile(File path, String filter, List<BentoSourceLoader> loaders, boolean recursive, boolean wait) {
        try {
            ...
            if (path.isDirectory() || isWildCard(path)) {
                ...
                if (load) {
                    log("Loading " + abspath + "...");
                    BentoSourceLoader fileLoader = new BentoSourceLoader(path, actions);
    ...
    private void loadURL(URL url, List<BentoSourceLoader> loaders, boolean wait) {
        try {
            ...
            if (load) {
                ...
                BentoSourceLoader urlLoader = new BentoSourceLoader(url, actions);
                urlLoader.load(wait);
    ...
    private void loadString(String src, List<BentoSourceLoader> loaders, boolean wait) {
        try {
            ...
            BentoSourceLoader srcLoader = new BentoSourceLoader(reader, actions);
            srcLoader.load(wait);
    ...
    private void link(List<BentoSourceLoader> loaders) {
...
//            
    ...
    private class BentoSourceLoader implements Runnable {
        ...
        public BentoSourceLoader(Object source, int actions) {
            this.source = source;
        ...
        private BentoParser getBentoParser() throws IOException {
            if (source instanceof Reader) {
                return new BentoParser((Reader) source);
            } else {
                return new BentoParser(getInputStream());
            }
        }
        ...
        public void run() {
            ...
            try {
                BentoParser parser = getBentoParser();
            ...
            } catch (bento.parser.TokenMgrError error) {
