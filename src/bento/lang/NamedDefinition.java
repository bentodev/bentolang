...
public class NamedDefinition extends AnonymousDefinition {
    ...
    public static NameNode createNameNode(String name) {
        ...
        // of BentoParser to properly parse the string and create whatever nodes
    ...
    private List<InsertStatement> inserts = null;
    transient private List<InsertStatement> insertsAndSuperInserts = null;
    ...
    public NamedDefinition(String name, BentoNode parent, Definition owner, Type superType, int access, int dur, Object value) {
        super();
    ...
    public Type getSuper(Context context, ParameterList params) {
        ...
        if (context != null && t != null) {
            Type insertedAbove = context.getInsertedAbove(t);
            if (insertedAbove != null) {
                st = insertedAbove;
                inserted = true;
            } else if (st != null) {
                Type insertedBelow = context.getInsertedBelow(st, t);
                if (insertedBelow != null && !insertedBelow.equals(getType())) {
                    st = insertedBelow;
                    inserted = true;
                }
            }
    ...
    public Type getSuperForChild(Context context, Definition childDef) throws Redirection {
        ...
        if (st == null) {
            ...
            while (it.hasNext()) {
                ...
                try {
                    Definition def = ((DefinitionInstance) sd.getChild(name, name.getArguments(), name.getIndexes(), null, context, false, false, null)).def;
    ...
    public Definition getChildDefinition(NameNode node, Context context) {
        ...
        while (n == 1) {
            BentoNode child = node.getChild(0);
...
//                    }
    ...
    public Object getChild(NameNode node, ArgumentList args, List<Index> indexes, ArgumentList parentArgs, Context context, boolean generate, boolean trySuper, Object parentObj) throws Redirection {
        ...
        if (node.numParts() == 1) {
            ...
            if (firstName.equals(Name.OWNER)) {
                int n = node.getNumChildren();
                node = new ComplexName(node, 1, n);
                DefinitionInstance defInstance = (DefinitionInstance) getOwnerInContext(context).getChild(node, node.getArguments(), node.getIndexes(), parentArgs, context, generate, trySuper, parentObj);
        ...
        if (isContainer) {
            ...
            if (container != null) {
                ...
                if (!node.isComplex()) {
                    ...
                    int n = node.getNumChildren();
                    node = new ComplexName(node, 1, n);
                    Object obj = container.getChild(node, node.getArguments(), node.getIndexes(), parentArgs, context, generate, trySuper, parentObj);
                    if (generate) {
        ...
        if (isAlias()) {
                ...
                if (!nameEqualsArg) {
                    ...
                    if (aliasDef != null) {
                        ...
                        try {
                            ...
                            numPushes++;
                            Object child = aliasDef.getChild(node, args, indexes, parentArgs, context, generate, trySuper, parentObj);
                            if ((generate && child != UNDEFINED) || (!generate && child != null)) {
                                return child;
                            }
            ...
            if (holder != null && holder.def != null && holder.def != this) {
                ...
                try {
                    return def.getChild(node, args, indexes, parentArgs, resolutionContext, generate, trySuper, parentObj);
                } finally {
        ...
        if (trySuper) {
            ...
            if (nd != null) {
                ...
                Object child = null;
                try {
                    child = nd.getChild(node, args, indexes, parentArgs, context, generate, trySuper, parentObj);
                    if ((!generate && child != null) || (generate && child != UNDEFINED)) {
                        return child;
                    }
                } finally {
     ...
     *  can be supported.nitialized.  See the Linker class in bento.lang.SiteLoader.
     */
    ...
    public void addInsert(InsertStatement insert) {
        if (inserts == null) {
            inserts = Context.newArrayList(1, InsertStatement.class);
        }
        inserts.add(insert);
    }

    public List<InsertStatement> getInserts() {
        return (insertsAndSuperInserts != null ? insertsAndSuperInserts : inserts);
    }

    protected void setInserts(List<InsertStatement> inserts) {
        this.inserts = inserts;
    }

    /** This method is called during the resolve pass when the site is initialized.  See
     *  the Resolver class in bento.runtime.SiteBuilder.
     */
    public void resolveInserts() {

        if (inserts != null) {
            insertsAndSuperInserts = Context.newArrayList(inserts);
        }
        NamedDefinition sdef = getSuperDefinition();
        if (sdef != null) {
            List<InsertStatement> superInserts = sdef.getInserts();
            if (superInserts != null) {
