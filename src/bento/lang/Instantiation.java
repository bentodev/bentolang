...
public class Instantiation extends AbstractConstruction implements ValueGenerator /*, ConstructionGenerator */ {
    ...
    protected void setReference(Object obj) {
        if (obj instanceof BentoNode) {
            reference = (BentoNode) obj;
        } else {
...
//                    && !((BentoNode) holder.nominalDef).isDynamic() 
...
//                            nm = name.getName();
//                            holder = context.getDefHolder(nm, nm, args, indexes, false);
//                            if (holder != null && holder.def != null && holder.def.getDurability() != Definition.DYNAMIC && !((BentoNode) holder.def).isDynamic() && !holder.def.equals(context.getDefiningDef())) {
//                                def = holder.def;
//                                args = holder.args;
//                                if (holder.data != null && holder.data != UNDEFINED) {
//                                    data = holder.data;
...
//            }
            ...
            // note: remove the exDef check if you want bento definitions
...
//                }
                if (!generate) {
                    ...
                    //numPushes += resolutionContext.pushParts(name, name.numParts(), getOwner());
...
//                    def = def1;
        ...
        if (def != null) {
            ...
            //        && !def.isCollection()
            //        && context.getCachedDefinition(def, args) == null
            //        && !name.isSpecial()) {
            //
...
//                    }
    ...
    private static Definition lookupSpecialName(NameNode nameNode, BentoNode parent, Definition owner, Context context) throws Redirection {
        int numUnpushes = 0;
        try {
            Definition def = null;
            ...
            if (name == Name.SUB) {
                ...
                Definition superdef = site.getDefinition("bento_context");
                Type superType = (superdef != null ? superdef.getType() : null);
                BentoContext bentoContext = new BentoContext(site, context);
                def = new ExternalDefinition(Name.HERE, parent, owner, superType, Definition.LOCAL_ACCESS, Definition.DYNAMIC, bentoContext, null);
             }
